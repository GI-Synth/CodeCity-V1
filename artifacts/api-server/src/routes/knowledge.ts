  ...
  const entries = [
    {
      ...
      language: "typescript",
      framework: "express",
      patternTags: JSON.stringify(["null_check", "async"]),
      fileType: "api",
    ...
    },
    {
      ...
      language: "javascript",
      framework: "unknown",
      patternTags: JSON.stringify(["race_condition", "async"]),
      fileType: "source",
    ...
    },
    {
      ...
      language: "python",
      framework: "fastapi",
      patternTags: JSON.stringify(["edge_case", "type_error"]),
      fileType: "api",
    ...
    },
    {
      ...
      language: "typescript",
      framework: "express",
      patternTags: JSON.stringify(["memory_leak", "api_abuse"]),
      fileType: "api",
      ...
      actionItems: JSON.stringify(["Profile with --inspect", "Check event listeners with EventEmitter.listenerCount", "Add cache size limits", "Use weak references for caches"]),
      confidence: "medium",
      provider: "claude",
      useCount: 3,
      wasUseful: 1,
    ...
    },
    {
      ...
      language: "python",
      framework: "pytest",
      patternTags: JSON.stringify(["sql_injection", "auth"]),
      fileType: "database",
...
router.get("/stats", async (_req, res) => {
  try {
    ...
    const topProblems = await db
      .select({
        type: knowledgeTable.problemType,
        count: count(),
      })
      .from(knowledgeTable)
...
router.get("/entries", async (_req, res) => {
  try {
    ...
    const entries = await db
      .select()
      .from(knowledgeTable)
      .orderBy(desc(knowledgeTable.useCount))
      .limit(20);
